...
struct internal_memory_builder_single_phf {
  ...
  build_timings build_from_keys(RandomAccessIterator keys,
    ...
    return build_from_hashes(
        hash_generator<RandomAccessIterator>(keys, config.seed), num_keys,
        config);
  }
  ...
  build_timings build_from_hashes(RandomAccessIterator hashes,
    ...
    time.mapping_ordering_microseconds =
        to_microseconds(clock_type::now() - start);
    if (config.verbose) {
      std::cout << " == mapping+ordering took "
                << time.mapping_ordering_microseconds / 1'000'000 << " seconds "
                << std::endl;
    ...
    if (config.verbose) {
      std::cout << " == search took " << time.searching_microseconds / 1'000'000
                << " seconds" << std::endl;
    }
  ...
  static uint64_t
  estimate_num_bytes_for_construction(const uint64_t num_keys,
                                      build_configuration const &config) {
    uint64_t table_size = static_cast<double>(num_keys) / config.alpha;
    const uint64_t num_buckets =
        (config.num_buckets == constants::invalid_num_buckets)
...
private:
  ...
  template <typename RandomAccessIterator> struct hash_generator {
  ...
  struct buckets_t {
    ...
    void print_bucket_size_distribution() {
      ...
      for (int64_t i = max_bucket_size - 1; i >= 0; --i) {
        ...
        std::cout << " == num_buckets of size " << t << " = "
                  << num_buckets_of_size_t << std::endl;
      }
    }
