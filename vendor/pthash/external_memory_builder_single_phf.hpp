  ...
  build_timings build_from_keys(Iterator keys, const uint64_t num_keys,
    ...
    if (num_buckets >= (1ULL << (sizeof(bucket_id_type) * 8))) {
      throw std::runtime_error(
          ...
          "-D PTHASH_ENABLE_LARGE_BUCKET_ID_TYPE=On' to change bucket_id_type "
          "to uint64_t or "
    ...
    if (bitmap_taken_bytes + hashed_pilots_cache_bytes >= ram) {
      ...
      ss << "not enough RAM available, the bitmap alone takes "
         << static_cast<double>(bitmap_taken_bytes) / 1'000'000'000
         << " GB of space.";
    ...
    try {
      ...
      if (config.verbose) {
        std::cout << " == map+ordering took "
                  << time.mapping_ordering_microseconds << " seconds"
                  << std::endl;
      }
    } catch (...) {
      tfm.remove_all_pairs_files();
    ...
    try {
      ...
      if (config.verbose) {
        std::cout << " == search took " << time.searching_microseconds
                  << " seconds" << std::endl;
      }
    } catch (...) {
      tfm.remove_all_pairs_files();
...
private:
  ...
  template <typename T> struct buffered_file_t : buffer_t<T> {
  ...
  struct temporary_files_manager {
    ...
    get_multifile_pairs_writer(uint64_t num_pairs, uint64_t ram,
  ...
  void map(Iterator keys, uint64_t num_keys, std::vector<pairs_t> &pairs_blocks,
    ...
    auto writer = tfm.get_multifile_pairs_writer(
        num_keys, ram - ram_parallel_merge, num_threads, ram_parallel_merge);
    try {
