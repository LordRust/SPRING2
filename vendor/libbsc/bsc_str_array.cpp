...
#define BSC_FILEOFFSET long
  ...
  int read_str_array(unsigned char *buf, int bsize) {
    ...
    while (true) {
      ...
      } else if (str_lengths[pos_in_str_array] - pos_in_current_str <=
                 (uint32_t)(bsize - bytes_written)) {
        memcpy(buf + bytes_written,
               str_array[pos_in_str_array].c_str() + pos_in_current_str,
  ...
  void write_str_array(unsigned char *buf, int bsize) {
    ...
    while (true) {
      ...
      if (pos_in_current_str == str_lengths[pos_in_str_array]) {
        pos_in_str_array++;
        pos_in_current_str = 0;
        if (pos_in_str_array >= size_str_array && bytes_read < bsize) {
  ...
  void Compression(char *argv[]) {
    ...
    int nBlocks = paramBlockSize > 0
                      ? (int)((fileSize + paramBlockSize - 1) / paramBlockSize)
                      : 0;
...

    {
      ...
      if (buffer == NULL) {
        {
          fprintf(stderr,
                  "Not enough memory! Please check README file for more "
                  "information.\n");
          throw std::runtime_error("BSC error.");
        }
      }
      ...
      while (true) {
        ...
        if (paramSortingContexts == LIBBSC_CONTEXTS_AUTODETECT) {
          sortingContexts =
              bsc_detect_contextsorder(buffer, dataSize, paramFeatures());
        ...
        int blockSize = bsc_compress(buffer, buffer, dataSize, paramLZPHashSize,
                                     paramLZPMinLen, paramBlockSorter,
                                     paramCoder, paramFeatures());
        if (blockSize == LIBBSC_NOT_COMPRESSIBLE) {
          {
            sortingContexts = LIBBSC_CONTEXTS_FOLLOWING;
            recordSize = 1;
        ...
        {
          BSC_BLOCK_HEADER header = {recordSize, sortingContexts};
      ...
      while (true) {
...

        {
          if ((feof(fInput) == 0) && (BSC_FTELL(fInput) != fileSize)) {
            ...
            if ((sortingContexts != LIBBSC_CONTEXTS_FOLLOWING) &&
                (sortingContexts != LIBBSC_CONTEXTS_PRECEDING)) {
              fprintf(
                  stderr,
                  "\nThis is not bsc archive or invalid compression method!\n");
              throw std::runtime_error("BSC error.");
            ...
            if (bsc_block_info(bscBlockHeader, LIBBSC_HEADER_SIZE, &blockSize,
                               &dataSize, paramFeatures()) != LIBBSC_NO_ERROR) {
              fprintf(
                  stderr,
                  "\nThis is not bsc archive or invalid compression method!\n");
              throw std::runtime_error("BSC error.");
            ...
            if (fread(buffer + LIBBSC_HEADER_SIZE,
                      blockSize - LIBBSC_HEADER_SIZE, 1, fInput) != 1) {
              fprintf(stderr, "\nUnexpected end of file: %s!\n", argv[2]);
              throw std::runtime_error("BSC error.");
            }
          }
        ...
        if (recordSize > 1) {
          result = bsc_reorder_reverse(buffer, dataSize, recordSize,
                                       paramFeatures());
          if (result != LIBBSC_NO_ERROR) {
            {
              switch (result) {
  ...
  void ShowUsage(void) {
    ...
    fprintf(stdout,
            "             -e2 Adaptive Quantized Local Frequency Coding (best "
            "compression)\n");

    fprintf(stdout, "\nPreprocessing options:\n");
    fprintf(stdout, "  -p       Disable all preprocessing techniques\n");
    fprintf(stdout,
            "  -s       Enable segmentation (adaptive block size), default: "
            "disable\n");
    fprintf(stdout,
...
public:
  int bsc_main(int argc, char *argv[], std::string *str_array_param,
...
void BSC_str_array_compress(const char *outfile, std::string *str_array_param,
  ...
  b.bsc_main(argv.size() - 1, argv.data(), str_array_param,
             size_str_array_param, str_lengths_param);
}
