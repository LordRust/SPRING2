...
- Fixed compression crashes (access violation exit code 0xC0000005) on Windows ClangCL x64 during plain FASTQ/FASTA compression. ClangCL's optimizer miscompiles `libsais.c` (the suffix array construction library used by libbsc for BWT-based compression) for complex real-world inputs with varied sequences and read IDs. Synthetic test data with simple cycling sequences and uniform quality values coincidentally avoids the affected code paths, so integration tests were unaffected while all smoke tests with real data failed. Applied the same `/Od` subdirectory-scope flag override that already existed for MSVC ARM64, now extended to ClangCL x64. Further investigation revealed the crash persists despite confirmed `/Od` application (8× slower smoke tests) — indicating the root cause is a stack overflow rather than miscompilation. With inlining disabled by `/Od`, `libsais.c`'s deeply recursive suffix array construction uses a full stack frame per call, and real genomic data drives far deeper recursion than synthetic cycling sequences. The default Windows thread stack (1 MB) is insufficient; the crash presents as `0xC0000005` (access violation) rather than `STATUS_STACK_OVERFLOW` because the guard page trips first. Fixed by linking `spring2.exe` with `/STACK:8388608` (8 MB) on all Windows MSVC-frontend builds (MSVC and ClangCL).
