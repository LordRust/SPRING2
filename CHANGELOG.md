...
- Added a small test FASTA reference (`ref_hg38_gencode49.fa`) under `data/reference/` and the corresponding metadata file for detecting assay type from input FASTQ files.
...
- Added a dedicated `scripts/` tooling directory for repository maintenance, including linting, cppcheck, cbindgen validation, Valgrind smoke checks, shared helpers, and suppressions.
