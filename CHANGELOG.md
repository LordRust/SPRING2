...
- Fixed gzipped input processing crashes on Windows ARM64 (access violation exit code 0xC0000005) by disabling optimizations for the cloudflare_zlib library on that platform. MSVC ARM64 compiler miscompilation in the zlib decompression path was causing access violations when reading `.gz` input files. The vendored `cloudflare_zlib` now compiles with `/Od` (no optimization) on Windows ARM64 to avoid the miscompilation, following the same pattern used for libbsc on that platform.
