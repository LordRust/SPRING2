...
- Fixed gzipped input processing crashes on Windows (access violation exit code 0xC0000005) by reducing optimization levels for the cloudflare_zlib library. MSVC ARM64, ClangCL, and regular Clang on Windows all exhibit aggressive optimization issues in the zlib inflate/gzread path that cause access violations when reading `.gz` input files, likely due to miscompilation or undefined behavior exploitation. The vendored `cloudflare_zlib` now compiles with `/O1` (MSVC/ClangCL) or `-O1` (Clang) instead of the default aggressive optimization levels, providing a safer middle ground between performance and correctness.
