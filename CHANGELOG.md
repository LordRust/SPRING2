...
- Fixed integration tests printing entire file contents (potentially megabytes of genomic data) to the test log on assertion failure. Doctest's expression decomposer stringifies both sides of a `CHECK(a == b)` comparison; for large binary strings this floods CI logs. Added a `check_bytes_equal` helper that pre-evaluates the comparison to a `bool` and reports only byte-sizes and a description on mismatch, and replaced all large-string file-content comparisons across the integration test suite with it.
...
- Fixed SPRING1 archive decompression producing truncated output on Windows ARM64. `decompress_short()` and `decompress_long()` used `#pragma omp parallel` with `omp_get_thread_num()` to assign each thread its block. Because OpenMP is disabled on MSVC ARM64 (compiler ICE in libsais), `omp_get_thread_num()` always returned 0, so only the first block of each step was decompressed and all remaining reads were left as empty strings. Replaced the `#pragma omp parallel` + `omp_get_thread_num()` pattern with `#pragma omp parallel for` over a loop index, so the fallback serial path still processes all blocks.
- Fixed gzipped input processing crashes on Windows (access violation exit code 0xC0000005) by disabling optimizations for the cloudflare_zlib library. MSVC, ClangCL, and regular Clang on Windows all exhibit aggressive optimization issues in the zlib inflate/gzread path that cause access violations when reading `.gz` input files, likely due to miscompilation or undefined behavior exploitation. The vendored `cloudflare_zlib` now compiles with `/Od` (no optimization) for MSVC/ClangCL or `-O1` (optimize for size) for Clang instead of the default aggressive optimization levels, ensuring correctness at the cost of some decompression performance.
- Fixed gzipped input processing crashes (access violation exit code 0xC0000005) on Windows ARM64 MSVC caused by ARM64-specific code paths in the vendored cloudflare_zlib being incorrectly activated. MSVC ARM64 (since VS 2019) defines `__aarch64__`, which cloudflare_zlib uses to enable CRC32 intrinsic code in `crc32.c` (`__crc32d`/`__crc32w` with raw `*(uint64_t *)` pointer casts near page boundaries) and NEON SIMD slide-hash in `deflate.c` (`vld1q_u16`/`vst1q_u16`). Both code paths contain unsafe unaligned pointer casts that cause access violations at runtime under MSVC. Fixed by passing `/U__aarch64__` to the cloudflare_zlib compile units on MSVC ARM64, forcing the generic safe C fallbacks for CRC32 and slide-hash.
- Fixed compression crashes (access violation exit code 0xC0000005) on Windows ClangCL x64 during compression of any input. The crashes affected 25 of 26 smoke tests; the only passing test used `-m 0.1` which capped threads to 1, making the critical parallel region execute on the main thread rather than an OpenMP worker — the decisive clue. The root cause is `thread_local std::string tl_plain_buffer` (declared at file scope in `io_utils.cpp`) being accessed inside `#pragma omp parallel` regions in `write_fastq_block` and `write_bgzf_fastq_block`. ClangCL (Clang frontend) implements C++ `thread_local` for types with non-trivial constructors using a dynamic TLS initialization wrapper — a per-thread constructor guard stored in a TLS slot. LLVM libomp creates its worker threads via `CreateThread`, which does not trigger the CRT's TLS callback/destructor mechanism for EXE-module dynamic TLS slots. Consequently, the first access to `tl_plain_buffer` in any OpenMP worker thread reads an uninitialized TLS slot and crashes with `0xC0000005`. MSVC uses `__declspec(thread)` (static TLS) where `CreateThread` does fire the TLS callback, explaining why MSVC x64 was unaffected. Fixed by removing the file-scope `thread_local` entirely and replacing it with a plain `std::string local_buf` declared inside each parallel block — inherently per-thread (stack-allocated on the worker's own stack), with no TLS dependency.
- Fixed remaining compression crashes (access violation exit code 0xC0000005) on Windows ClangCL x64 after the `thread_local` fix. LLVM libomp creates its OpenMP worker threads via the raw Win32 `CreateThread()` API rather than `_beginthreadex()`. With the static CRT (`/MT`), the MSVC per-thread runtime state (`_tiddata`, which backs C++ exception-handling frames, locale, and other CRT facilities) is only initialised via `DllMain(DLL_THREAD_ATTACH)` for DLL-resident code — it is not triggered for `CreateThread`-created threads when the CRT is statically linked into the EXE. MSVC's OpenMP runtime (vcomp.dll) uses `_beginthreadex()` internally and therefore always initialises per-thread CRT state; LLVM libomp does not. Clang generates exception-handling frames that interact with uninitialised `_tiddata` in a way that triggers an access violation as soon as control enters a non-trivial C++ scope in a worker thread, including the first `#pragma omp parallel` region during compression. Fixed by switching the ClangCL build to the dynamic CRT (`/MD`): `vcruntime140.dll`'s `DllMain(DLL_THREAD_ATTACH)` is then called for all threads regardless of how they were created, fully initialising per-thread state before any user code runs. MSVC continues to use the static CRT (`/MT`) because its vcomp.dll already handles this correctly.
- Fixed integration test failures on Windows (`tar: _repack_tmp.tar: Can't add archive to itself`, exit code 1) caused by Windows bsdtar creating the output archive before processing the `*` glob, so the newly-created file gets included in the glob and bsdtar exits with an error. Added `--exclude _repack_tmp.tar` to all `tar -cf` invocations so the output file is excluded from glob expansion on all platforms.
