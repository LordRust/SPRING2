...
- Fixed integration test tar warnings (`tar: _repack_tmp.tar: Can't add archive to itself`) by explicitly removing the temporary tar file before creating it in corruption and metadata-modification test cases. The tests extract archives, modify files, and repack them with `tar -cf _repack_tmp.tar *`; if the temporary tar file already existed from a previous run, the `*` glob would include it, causing tar to emit warnings. Now each test explicitly removes `_repack_tmp.tar` before invoking tar.
