...
- Implemented chunked reorder: datasets exceeding 50 million reads are now split into chunks of up to 50 M reads and reordered sequentially. Each chunk allocates roughly 2 GB of bitset memory (instead of the full dataset's working set), keeping MPHF lookups and Hamming-distance checks within the L3 cache / DRAM bandwidth budget. For a 726 M-read sc-ATAC dataset that previously required ~12 hours for the reorder pass, the expected runtime drops to ~100 minutes (15 chunks × ~400 s/chunk). Reads within each chunk are assigned globally-contiguous IDs before the results are merged, so quality and ID reordering downstream sees an unbroken ID sequence. A `SPRING2_REORDER_CHUNK_SIZE` environment variable can override the 50 M threshold (intended for testing).
- Fixed a second memory bottleneck in the chunked reorder path for disk_path mode: `clean_read_streams` (~29 GB for 726 M reads) and per-chunk `singleton_read_bytes` (~7.4 GB × 15 chunks) were previously kept in RAM simultaneously, causing the job to be OOM-killed at chunk 9. In disk_path mode the chunk loop now spills both streams to disk immediately (freeing ~29 GB) and flushes `singleton_read_bytes` per chunk to the reorder artifact directory rather than accumulating them in memory. The artifact directory is produced in the same layout as `spill_reorder_encoder_artifact` so the encoder stage loads it unchanged. Peak RAM during the reorder stage is now ~15–20 GB regardless of total read count.
- Fixed an encoder OOM in disk_path mode for datasets with a large fraction of unmatched singletons (e.g. sc-ATAC). After the main OMP encoding loop, `write_unaligned_range` was accumulating all remaining-unaligned reads into `unaligned_serialized` in RAM before writing to `unaligned_serialized.bin`. For the 726 M-read sc-ATAC dataset (~657 M unmatched singletons × 155 bytes each) this produced a ~102 GB in-memory buffer on top of the ~33 GB singleton bitset pool, triggering an OOM kill at "Merging encoder metadata to disk...". A new `write_unaligned_range_streaming` variant now writes each unaligned read's order, length, and sequence bytes directly to the three output file streams without any intermediate accumulation, eliminating the peak ~102 GB allocation.
