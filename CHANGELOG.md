...
- Fixed gzipped input processing crashes on Windows with MSVC-like compilers (access violation exit code 0xC0000005) by disabling optimizations for the cloudflare_zlib library. MSVC ARM64 and ClangCL compiler miscompilation in the zlib inflate/deflate path was causing access violations when reading `.gz` input files. The vendored `cloudflare_zlib` now compiles with `/Od` (no optimization) on Windows for MSVC ARM64 and ClangCL to avoid the miscompilation, following the same pattern used for libbsc on MSVC ARM64.
