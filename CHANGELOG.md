...
- Fixed gzipped input processing crashes on Windows with MSVC-like compilers (access violation exit code 0xC0000005) by reducing optimization levels for the cloudflare_zlib library. MSVC ARM64 and ClangCL compiler aggressive optimizations in the zlib inflate/gzread path were causing access violations when reading `.gz` input files, likely due to miscompilation or undefined behavior exploitation. The vendored `cloudflare_zlib` now compiles with `/O1` (optimize for size) instead of the default `/O2` (optimize for speed) on Windows for MSVC ARM64 and ClangCL, providing a safer middle ground between performance and correctness.
