...
- Fixed compression crashes (access violation exit code 0xC0000005) on Windows ClangCL x64 during plain FASTQ/FASTA compression. ClangCL's optimizer miscompiles `libsais.c` (the suffix array construction library used by libbsc for BWT-based compression) for complex real-world inputs with varied sequences and read IDs. Synthetic test data with simple cycling sequences and uniform quality values coincidentally avoids the affected code paths, so integration tests were unaffected while all smoke tests with real data failed. Applied the same `/Od` subdirectory-scope flag override that already existed for MSVC ARM64, now extended to ClangCL x64. Further investigation revealed the crash persists despite confirmed `/Od` application (8× slower smoke tests) — indicating the root cause is a stack overflow in OpenMP worker threads rather than miscompilation. libsais uses `#pragma omp parallel` internally; LLVM libomp (used by ClangCL) defaults to a 4 MB per-thread stack. With `/Od` disabling inlining, each SA-IS recursive call uses a full stack frame, and real genomic data (25 MB blocks, complex varied sequences) drives the recursion deep enough to overflow 4 MB while synthetic cycling data does not. Fixed by calling `omp_set_stacksize(64 MB)` at the start of `main()` before any parallel regions are entered, raising the worker stack to 64 MB for all builds.
